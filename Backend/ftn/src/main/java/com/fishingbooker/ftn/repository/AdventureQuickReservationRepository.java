...
public interface AdventureQuickReservationRepository extends EntityRepository<AdventureQuickReservation> {
    ...
    @Query(value = "SELECT *\n" +
            "\tFROM public.adventure_quick_reservation natural join public.quick_reservation\n" +
            "where adventure_id=:id and  ((:startDate between action_start and action_end) or (:endDate  between action_start and action_end) or (action_start between :startDate and :endDate) or (action_end between :startDate and :endDate))", nativeQuery = true)
