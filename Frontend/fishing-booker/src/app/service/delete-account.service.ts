...
export class DeleteAccountService {
    ...
    approveRequest(id:string,description:string):void{
        ...
        this._http.put<any>(url,{id:id,description:description},{headers:headers}).subscribe(console.log);
      }
    ...
    sendRequest(description:string):void{
      ...
      const headers=this.loginService.getHeaders();
      const url=server+'api/delete-account';
      this._http.post<any>(url,{userId:userId,description:description},{headers:headers}).subscribe(data=>{
        if (data==false){
