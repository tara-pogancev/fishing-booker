...
@Component({
  selector: 'app-cottage-owner-edit-cottage',
  templateUrl: './cottage-owner-edit-cottage.component.html',
  styleUrls: ['./cottage-owner-edit-cottage.component.css']
})
export class CottageOwnerEditCottageComponent implements OnInit {
  ...
  constructor(private http: HttpClient, private utilityService: UtilityService, private cottageService: CottageService, private route: ActivatedRoute) { }
  ...
  validateForm() {
    if (this.cottage.name != "" && this.cottage.guestLimit > 0 && this.cottage.description != "" && this.cottage.city != "" && this.cottage.country != "" && this.cottage.price >= 0 && this.cottage.street != "") {
      ...
      alert('Cottage successfully changed!');
      window.location.href = "preview-cottage/" + this.cottage.id;
    } else {
